    ...
    <div class="grid gap-16 justify-items-center">
      <div class="grid gap-6 text-center lg:w-4/5">
        <h1 class="font-semibold text-4xl lg:text-5xl">
          <span class="text-primary-400">{{ t('test-mission.heading-1') }}</span> <span
            class="text-gray-900"
          >{{ t('test-mission.heading-2') }}</span>
        </h1>
      </div>
      <div
        class="relative overflow-hidden w-fit mx-auto rounded-2xl"
        ...
        <div class="relative w-fit h-fit" :class="{ 'lg:order-2': (index) % 2 }">
          ...
          >
            <img
              :src="mission.image"
              class="h-full"
              :class="mission.device ? 'object-cover' : 'object-contain'"
            >
          </div>

  ...
  <div class=" grid gap-4 h-full">
      ...
      <ol role="list" class="flex grow items-center">
        <li v-for="(step, stepIdx) in steps" :key="step.name" class="relative grow grid" :class="[stepIdx !== steps.length - 1 ? 'pr-16' : '']">
          <template v-if="step.status === 'complete'">
            ...
            <span class="relative w-8 h-8 flex items-center justify-center mx-auto border-4 border-white rounded-full" :class="(stepIdx + 1) % 2 ? 'bg-primary-100' : 'bg-tertiary-100'">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16">
      <div v-for="(step, sId) in steps" :key="sId" class="grid gap-3 lg:gap-6 content-start justify-items-center  text-black text-center">
        ...
        <button v-if="sId === 0" id="cta-testuser-form-3" class="bg-primary-500 text-medium text-base text-white w-fit flex items-center py-4 px-4 rounded-lg gap-2" @click="open">

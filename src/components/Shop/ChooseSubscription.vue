  ...
  <div id="choose-subscription" class="w-full grid gap-8 content-start p-6">
    ...
    <div class="flex justify-center items-center gap-3">
      ...
      <span class=" font-normal lg:font-medium text-gray-700">
        Jährlich im Voraus zahlen* <span class="text-primary-500" @click="showInformation = true">(spare 20%)
          <div class="i-heroicons-outline:information-circle align-middle inline-block w-6 h-6 shrink-0" />
        </span>
      </span>
    </div>
    ...
    <div>
      <RadioGroup v-model="selectedSubscription">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-3">
          <RadioGroupOption v-for="subscription in subscriptions" :key="subscription.id" v-slot="{ checked }"
            as="template" :value="subscription" @click="setCart">
            <div
              ...
              <div class="grid gap-2">
                ...
                <RadioGroupLabel as="span" class="block text-lg font-normal transition-all transition-duration-500"
    ...
    <FormKit type="form" form-class="grid gap-4" :actions="false" :incomplete-message="false" @submit="buyNow">
      <FormKit id="7539405" type="submit"
      ...
      <FormKit v-model="isAgbAccepted" type="checkbox" label-class="text-sm flex-1" wrapper-class="flex items-start gap-2"
        ...
        validation-label="AGBs" message-class="text-red-500 mt-1" validation="required|accepted"
        validation-visibility="submit">
        <template #label="context">
          <span :class="context.classes.label">Ich akzeptiere die <router-link to="/agb"
              class="underline underline-primary-500 text-primary-500">Allgemeinen Geschäftsbedingungen</router-link>,
            und die <router-link to="/agb" class="underline underline-primary-500 text-primary-500">Widerrufsbelehrung
            </router-link>.*</span>
        </template>
      </FormKit>
      <FormKit v-model="isNewsletterAccepted" type="checkbox" label-class="text-sm flex-1"
    ...
    <transition enter-active-class="transition-all duration-1000 ease-in-out"
      enter-from-class="transform translate-y-5 opacity-0" leave-active-class="transition-all duration-300 ease-out"

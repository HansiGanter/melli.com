<script setup lang="ts">import { fireDipaFormOpenEvent } from '~/google-tag-manager';
    ...
    <div class="absolute top-0 w-full h-full bg-black/50 flex flex-col">
      ...
      <div class="my-auto flex flex-col gap-5 sm:gap-9 py-12 px-5 max-w-160 mx-auto">
        ...
        <div class="flex flex-col gap-5 sm:gap-4 sm:items-center">
          <button class="rounded-lg bg-primary-400 text-white py-2.5 px-4 flex items-center w-fit gap-2 justify-center" @click="openForm()">
            <div class="i-carbon:user-favorite-alt-filled w-6 h-6" /><span>Melli-Tester werden</span>
          </button>
        </div>
      </div>
      <RouterLink to="#dipa-quote" class="absolute bottom-6 font-normal text-lg text-white flex flex-col gap-2 w-full mx-auto items-center">
        mehr erfahren
        <img
